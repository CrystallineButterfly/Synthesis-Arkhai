...
MODULE = importlib.import_module('agents.arkhai_accord')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'arkhai_accord')
        self.assertEqual(spec.primary_contract_name, 'GrantAccordEscrow')
