...
contract GrantAccordEscrowTest is Test {
    GrantAccordEscrow private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_arkhai");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new GrantAccordEscrow(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        GrantAccordEscrow freshController = new GrantAccordEscrow(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
